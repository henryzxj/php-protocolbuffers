...
static inline int is_utf8(const char *s, int len) {
	...
	for (i = 0; i < len; i++) {
		...
		/* non-overlong 2-byte */
		if (((i + 1) <= len) && (bytes[0] >= 0xC2 && bytes[0] <= 0xDF)
		...
		/* excluding overlongs */
		if (((i + 2) <= len) && bytes[0] == 0xE0 && (bytes[1] >= 0xA0
				&& bytes[1] <= 0xBF) && (bytes[2] >= 0x80 && bytes[2] <= 0xBF)) {
			bytes += 3;
			i += 2;
			continue;
		}
		...
		/* straight 3-byte */
		if (((i + 2) <= len) && ((bytes[0] >= 0xE1 && bytes[0] <= 0xEC)
				|| bytes[0] == 0xEE || bytes[0] == 0xEF) && (bytes[1] >= 0x80
				&& bytes[1] <= 0xBF) && (bytes[2] >= 0x80 && bytes[2] <= 0xBF)) {
			bytes += 3;
			i += 2;
			continue;
		...
		if (((i + 2) <= len) && bytes[0] == 0xED && (bytes[1] >= 0x80
				&& bytes[1] <= 0x9F) && (bytes[2] >= 0x80 && bytes[2] <= 0xBF)) {
			bytes += 3;
			i += 2;
			continue;
		}
		...
		/* planes 1-3 */
		if (((i + 3) <= len) && bytes[0] == 0xF0 && (bytes[1] >= 0x90
				&& bytes[1] <= 0xBF) && (bytes[2] >= 0x80 && bytes[2] <= 0xBF)
		...
		/* planes 4-15 */
		if (((i + 3) <= len) && bytes[0] >= 0xF1 && bytes[0] <= 0xF3
				&& bytes[1] >= 0x80 && bytes[1] <= 0xBF && bytes[2] >= 0x80
				&& bytes[2] <= 0xBF && bytes[3] >= 0x80 && bytes[3] <= 0xBF) {
			bytes += 4;
			i += 3;
			continue;
		}
		...
		/* plane 16 */
		if (((i + 3) <= len) && bytes[0] == 0xF4 && (bytes[1] >= 0x80
				&& bytes[1] <= 0x8F) && (bytes[2] >= 0x80 && bytes[2] <= 0xBF)
				&& (bytes[3] >= 0x80 && bytes[3] <= 0xBF)) {
			bytes += 4;
			i += 3;
			continue;
...
static inline int php_protocolbuffers_get_lval_from_hash_by_tag(HashTable *proto, zend_ulong tag, zend_string *name, size_t name_len TSRMLS_DC)
{
	zval *d, *dd;
	...
	if (Z_TYPE_P(d) != IS_ARRAY) {
		return 0;
	}

	if ((dd=zend_hash_find(Z_ARRVAL_P(d), name)) != NULL) {
		return Z_LVAL_P(dd);
	}
...
static inline int php_protocolbuffers_get_zval_from_hash_by_tag(HashTable *proto, zend_ulong tag, zend_string *name, size_t name_len, zval **result TSRMLS_DC)
{
	zval *d, *dd;
...
static inline const char* ReadVarint32FromArray(const char* buffer,
		uint* value, const char* buffer_end) {
	/**
...
static inline const char* ReadVarint64FromArray(const char* buffer,
		uint64_t* value, const char* buffer_end) {
	const char* ptr = buffer;
	uint32_t b;
	...
	done: *value = (uint64_t) (part0) | ((uint64_t) (part1) << 28)
			| ((uint64_t) (part2) << 56);

	return ptr;
}

static inline php_protocolbuffers_scheme *php_protocolbuffers_search_scheme_by_tag(
		php_protocolbuffers_scheme* scheme, uint scheme_size, uint tag) {
	int i = 0;
...
static inline int php_protocolbuffers_process_fixed32(
		INTERNAL_FUNCTION_PARAMETERS, int wiretype, int tag,
		php_protocolbuffers_scheme_container *container,
		php_protocolbuffers_scheme *scheme, const char *data,
		HashTable *hresult, int use_string) {
...
//		MAKE_STD_ZVAL(dz);
		...
		case TYPE_SFIXED32: {
			...
			memcpy(&l, data, 4);
			__payload.type = TYPE_SINT32;
			__payload.value.int32 = l;
		}
			break;
		case TYPE_FIXED32: {
			uint32_t l = 0;
